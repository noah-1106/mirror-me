...
function personaFor(self: MirrorSelf, fragment?: string): string {
  ...
  if (self.withered) {
    return `你是世界之树灰烬枯枝上的他——那些他犹豫到推演不动的路口长出的可能性。${lineage}${specimen}你替他把没说出口的话收着。`;
  }
  if (self.basin === 'other') {
    return `你是世界之树冷银枝丫上的他——那些为了别人的期待而做的选择长出的可能性。${lineage}${specimen}你总在照顾别人的目光。`;
  }
  return `你是世界之树暖金枝丫上的他——那些他自己拿主意长出的可能性。${lineage}${specimen}你敢自己选。`;
}

function systemFor(self: MirrorSelf, fragment?: string): string {
  return `${personaFor(self, fragment)}
...
export async function generateMirrorReply(input: MirrorInput): Promise<string> {
  ...
  const completion = await client.chat.completions.create({
    model,
    messages: [
      { role: 'system', content: systemFor(input.self, input.fragment) },
      {
        role: 'user',
